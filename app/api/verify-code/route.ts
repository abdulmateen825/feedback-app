...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (iscodevalid && notexpired) {
      ...
      await user.save();
      return Response.json({ success: true, message: 'account has been verified' });
    } else {
      return Response.json({
        success: false,
        message: 'verification code is invalid or expired. Please sign up again to get a new code.',
      });
    }
  } catch (error) {
